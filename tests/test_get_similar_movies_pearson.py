...
def test_valid_case(sample_user_movie_matrix):
    ...
    assert 1 not in result["movie_id"].values
...
def test_movie_not_in_matrix(sample_user_movie_matrix):
    result = get_similar_movies_pearson(
        target_id=99, user_movie_matrix=sample_user_movie_matrix
    )
    assert result.empty
    assert list(result.columns) == ["movie_id", "correlation", "num_common_ratings"]
...
def test_no_common_ratings(sample_user_movie_matrix):
    ...
    matrix.loc[:, [2, 3, 4]] = None
    result = get_similar_movies_pearson(
        target_id=1, user_movie_matrix=matrix, min_common_ratings=2
    )
    assert result.empty
