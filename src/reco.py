...
# === Fonction de reco ===
def recommend_similar_movies(film_title, movie_genres,
                             genre_columns, top_n=5):
    ...
    return similar_movies.sort_values(
            by="similarity", ascending=False)[
                ["title", "similarity"]].head(top_n)


def recommend_by_user_ratings(film_title, user_movie_matrix, ratings,
                              min_ratings=50, top_n=5):
    ...
    filtered_corr = filtered_corr.sort_values(by="correlation",
                                              ascending=False)
    # Top recommandations
