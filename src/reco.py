...
def recommend_similar_movies(
    ...
    selected: pd.DataFrame = movie_genres[movie_genres["title"] == movie_title]
    if selected.empty:
    ...
    similar_movies = movie_genres[movie_genres["title"] != movie_title]
...
def recommend_by_user_ratings(
    ...
    # --- Map title -> movie_id ---
    movie_row: pd.Series = movies[movies["title"] == movie_title]
    if movie_row.empty:
        raise KeyError(
            f"The movie '{movie_title}' does not exist in the movies database."
        )
    movie_id: int = int(movie_row["movie_id"].iloc[0])

    # --- Verify if the matrix is index by ids or by title ---
    if movie_id in user_movie_matrix.columns:
        col_key = movie_id
    elif movie_title in user_movie_matrix.columns:
        col_key = movie_title
    else:
        raise ValueError(f"No rating find for '{movie_title}' (id={movie_id}).")

    # --- ratings of movie_title ---
    ...
    # --- Map index -> title ---
    if user_movie_matrix.columns.dtype == "O":  # columns = titles
        filtered_corr: pd.DataFrame = filtered_corr.merge(
            movies, left_index=True, right_on="title"
        )
    else:  # columns = IDs
        filtered_corr: pd.DataFrame = filtered_corr.merge(
    ...
    filtered_corr: pd.DataFrame = filtered_corr[filtered_corr["title"] != movie_title]

    return filtered_corr[["movie_id", "title", "correlation", "num_ratings"]].head(
        top_n
    )
